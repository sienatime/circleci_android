...
public class CircleService {
  ...
  private Gson gson = new GsonBuilder().setDateFormat("yyyy-MM-dd'T'HH:mm:ss.S'Z'").create();

  public CircleService(Bus bus, String token) {
    this.bus = bus;
    this.restAdapter = new RestAdapter.Builder().setEndpoint("https://circleci.com")
        .setRequestInterceptor(new CircleRequestInterceptor(token))
        .build();
    this.api = restAdapter.create(CircleAPI.class);
  }

  @Subscribe public void getBuilds(LoadBuildsEvent event) {
    Context context = CircleApp.getInstance().getApplicationContext();
    Toast.makeText(context, context.getResources().getString(R.string.refreshing),
        Toast.LENGTH_SHORT).show();
    api.listBuilds(new Callback<Object>() {
      @Override public void success(Object builds, Response response) {
        Build[] buildArray = gson.fromJson(gson.toJson(builds), Build[].class);
  ...
  @Subscribe public void retryBuild(RetryBuildEvent event) {
    api.retryBuild(event.getProject(), event.getUsername(), event.getBuildNumber(),
        new Callback<Object>() {
  ...
  @Subscribe public void cancelBuild(CancelBuildEvent event) {
    api.cancelBuild(event.getProject(), event.getUsername(), event.getBuildNumber(),
        new Callback<Object>() {
          @Override public void success(Object object, Response response) {
            Build build = gson.fromJson(gson.toJson(object), Build.class);
